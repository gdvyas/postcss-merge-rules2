...
function selectorMerger(browsers, compatibilityCache) {
    ...
    return function (rule) {
        if (cacheList[rule.selector]) {
            ...
            var cached = getDecls(cacheList[rule.selector]);
        ...
        for (var name in cacheList) {
            ...
            if (!canMerge(rule, _cache, browsers, compatibilityCache)) {
                continue;
            }
            // Merge when declarations are exactly equal
            // e.g. h1 { color: red } h2 { color: red }
            if (getDecls(rule).join(';') === getDecls(_cache).join(';')) {
                delete cacheList[_cache.selector];
                delete cacheList[rule.selector];
                rule.selector = joinSelectors(_cache, rule);
                _cache.remove();
                cacheList[rule.selector] = rule;
                continue;
            }
            ...
            partialMerge(_cache, rule, cacheList);
        }
    };
}
